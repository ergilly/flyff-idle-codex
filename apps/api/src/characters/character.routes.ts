...
async function purchaseShopItem(request: Request, response: Response, townMapId: string, locationId: string) {
  ...
  const purchaseResult = shopPurchaseSchema.safeParse(request.body);

  if (!characterIdResult.success || !purchaseResult.success) {
    response.status(400).json({ error: "Shop item is required" });
    return;
  }

  const stockItem = getTownShopStockItem(townMapId, locationId, purchaseResult.data.itemId);
  ...
  if (purchaseResult.data.quantity > maxStack) {
  ...
  const result = characterRepository.purchaseInventoryItemForPlayer(
    ...
    purchaseResult.data.itemId,
    purchaseResult.data.quantity,
    unitPrice,
    maxStack
  );
  ...
  if (!result.character) {
    response
      ...
      .json({ error: result.error ?? "Unable to purchase item" });
    return;
  }
...
characterRouter.post(
  ...
  async (request, response) => {
    await purchaseShopItem(request, response, "flarine-town", "general-store");
  }
);

characterRouter.post("/:characterId/shops/sales", requireAuth, async (request, response) => {
  const characterIdResult = z.string().safeParse(request.params.characterId);
  const saleResult = shopSaleSchema.safeParse(request.body);

  if (!characterIdResult.success || !saleResult.success) {
    response.status(400).json({ error: "Inventory item and quantity are required" });
    return;
  }

  const auth = response.locals.auth as AuthTokenPayload;
  const character = characterRepository.findById(characterIdResult.data);
  const inventoryItem = character?.inventory.items.find(
    (item) => item.slotIndex === saleResult.data.slotIndex
  );
...
characterRouter.post(
  ...
  async (request, response) => {
    await purchaseShopItem(
      request,
      response,
      String(request.params.townMapId),
      String(request.params.locationId)
    );
  }
);
