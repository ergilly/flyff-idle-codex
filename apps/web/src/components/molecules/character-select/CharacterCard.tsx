...
export function CharacterCard({ character, onCreate, onDelete, onSelect, slotNumber }: CharacterCardProps) {
  if (!character) {
    return (
      <article
        ...
        data-testid={`characters_article_empty_slot_${slotNumber}`}
      >
        <button
          ...
          data-testid={`characters_button_create_slot_${slotNumber}`}
          type="button"
  ...
  return (
    <article
      ...
      data-testid={`characters_article_slot_${character.slotIndex}`}
    >
      <button
        ...
        data-testid={`characters_button_delete_${character.slotIndex}`}
      ...
      <button
        ...
        data-testid={`characters_button_select_${character.slotIndex}`}
        type="button"
      ...
      >
        <div
          ...
          data-testid={`characters_div_class_icon_${character.slotIndex}`}
        >
          <Image
        ...
        <div data-testid={`characters_div_summary_${character.slotIndex}`}>
          <h2 data-testid={`characters_h2_name_${character.slotIndex}`}>{character.name}</h2>
          <MutedText data-testid={`characters_p_job_${character.slotIndex}`}>{character.job}</MutedText>
        </div>
        <Stack data-testid={`characters_div_stats_${character.slotIndex}`}>
          <StatRow
            data-testid={`characters_stat_level_${character.slotIndex}`}
            label="Level"
            value={character.level}
          />
          <StatRow
            data-testid={`characters_stat_exp_${character.slotIndex}`}
            label="EXP"
            value={character.exp}
          />
        </Stack>
